...
- **Tenant actual:** La entidad **Org** actúa como tenant. Casi todo lo operativo está ligado a `orgId`: Client, Project, Quote, Sale, Warehouse, CountryProfile, FreightRateProfile, TaxRuleSet, BillingEntity, Payment, AuditLog, RevitImport.
- **Global (sin orgId):** User, SystemType, PieceCatalog, PieceCost. Son datos de catálogo o usuarios del sistema.
- **Híbrido:** PieceAlias tiene `orgId` opcional (alias por org o global).
...
└───┬─────────────────────────────────────────────────────────────────────────────────────────┘
    │
    ├──► OrgMember (orgId, userId, role) ◄── User (id, email, name, passwordHash, status)
    ├──► Client (orgId, name, legalName, taxId, address, city, countryId, …)
    ├──► Project (orgId, clientId?, name, status, baselineQuoteId?, soldAt, finalAmountUsd, …)
    ├──► Quote (orgId, projectId, countryId?, status, factoryCostUsd, commissionPct/Fixed, …)
    ├──► Warehouse (orgId, name, location)
    ├──► CountryProfile (orgId, code, name, currency)
    ├──► FreightRateProfile (orgId, countryId, freightPerContainer, …)
    ├──► TaxRuleSet (orgId, countryId, name, rules JSON)
    ├──► BillingEntity (orgId, name, slug)
    ├──► Sale (orgId, clientId, projectId, quoteId?, status, exwUsd, commissionPct, …)
    ├──► Payment (orgId, saleId, entityId, amountUsd, paidAt, …)
    ├──► RevitImport (orgId, projectId?, filename, status, …)
    └──► AuditLog (orgId?, userId?, action, entityType, entityId, meta)
...
│ PieceAlias (orgId?, pieceId, aliasRaw, aliasNormalized, source)  ← orgId opcional           │
└─────────────────────────────────────────────────────────────────────────────────────────────┘
...
| **org_members** | id, orgId, userId, role (OrgMemberRole), createdAt, updatedAt | N:1 Org, N:1 User. UNIQUE(orgId, userId) |
| **audit_logs** | id, orgId?, userId?, action, entityType?, entityId?, meta (JSON), createdAt | N:1 Org, N:1 User |

### Clientes y proyectos

| Tabla | Atributos principales | Relaciones |
|-------|------------------------|------------|
| **clients** | id, orgId, name, legalName, taxId, address, city, countryId, phone, email, website, notes, createdAt, updatedAt | N:1 Org, N:1 CountryProfile; 1:N Project, Sale |
| **projects** | id, orgId, name, client?, clientId?, location, countryId?, description, wallAreaM2*, kitsPerContainer, numContainers, totalKits, plannedStartDate, durationWeeks, isArchived, status (ProjectStatus), baselineQuoteId?, soldAt?, finalAmountUsd?, createdAt, updatedAt | N:1 Org, N:1 Client, N:1 CountryProfile, N:1 Quote (baseline); 1:N RevitImport, Quote, Sale |
...
| **piece_aliases** | id, orgId?, pieceId, aliasRaw, aliasNormalized, source?, createdAt | N:1 PieceCatalog. UNIQUE(aliasNormalized, pieceId). orgId opcional → alias por org o global |

### Países, flete e impuestos (por org)

| Tabla | Atributos principales | Relaciones |
|-------|------------------------|------------|
| **country_profiles** | id, orgId, code, name, currency, isActive, createdAt, updatedAt | N:1 Org; 1:N Client, FreightRateProfile, TaxRuleSet, Project, Quote. UNIQUE(orgId, code) |
| **freight_rate_profiles** | id, orgId, countryId, name, freightPerContainer, isDefault, validFrom, expiryDate?, notes, createdAt, updatedAt | N:1 Org, N:1 CountryProfile |
...
| Tabla | Atributos principales | Relaciones |
|-------|------------------------|------------|
| **revit_imports** | id, orgId, projectId?, filename, uploadedBy?, rowCount, matchedCount, unmappedCount, status, createdAt, updatedAt | N:1 Org, N:1 Project; 1:N RevitImportLine |
...
| Tabla | Atributos principales | Relaciones |
|-------|------------------------|------------|
| **warehouses** | id, orgId, name, location?, isActive, createdAt, updatedAt | N:1 Org; 1:N InventoryItem, InventoryMove (from/to) |
...
| Tabla | Atributos principales | Relaciones |
|-------|------------------------|------------|
| **billing_entities** | id, orgId, name, slug, isActive, createdAt, updatedAt | N:1 Org; 1:N SaleInvoice, Payment. UNIQUE(orgId, slug) |
| **sales** | id, orgId, clientId, projectId, quoteId?, saleNumber?, quantity, status (SaleStatus), exwUsd, commissionPct, commissionAmountUsd, fobUsd, freightUsd, cifUsd, taxesFeesUsd, landedDdpUsd, invoicedBasis?, taxBreakdownJson?, notes?, createdBy?, createdAt, updatedAt | N:1 Org, Client, Project, Quote?, User (createdBy); 1:N SaleInvoice, Payment. UNIQUE(orgId, saleNumber) |
...
| **payments** | id, orgId, saleId, entityId, amountUsd, amountLocal?, currencyLocal?, exchangeRate?, paidAt, notes?, createdBy?, createdAt | N:1 Org, Sale (CASCADE), BillingEntity, User (createdBy) |

---

## Índices relevantes (del schema)

- org_members: (orgId, userId) UNIQUE  
- clients: orgId, countryId, name  
- projects: orgId, clientId, countryId, status, soldAt  
- quotes: orgId, projectId  
- sales: orgId, clientId, projectId, status, createdAt; (orgId, saleNumber) UNIQUE  
- country_profiles: (orgId, code) UNIQUE  
- billing_entities: (orgId, slug) UNIQUE  
- audit_logs: orgId, userId, action  
...
- **Client, Project, Quote, Sale, Payment, BillingEntity:** todos con `orgId` → cada partner (org) ve solo los suyos filtrando por org.
- **Warehouse, CountryProfile, FreightRateProfile, TaxRuleSet, RevitImport:** por org.
- **AuditLog:** tiene `orgId` opcional; se puede seguir filtrando por org para partners.
...
2. **Catálogo y costos**  
   - **PieceCatalog** y **PieceCost** son globales (sin orgId).  
...
4. **Filtro en APIs y UI**  
   - Para que “cada partner vea solo lo suyo”, hay que asegurar que todas las consultas de Client, Project, Quote, Sale, etc. filtren por `orgId` según la Org del usuario en sesión (y que el superadmin, cuando esté definido, pueda omitir o ampliar ese filtro).
...
4. Revisar todas las rutas/APIs que lean Client, Project, Quote, Sale, etc. y garantizar el filtro por `orgId` (y excepción controlada para superadmin).
