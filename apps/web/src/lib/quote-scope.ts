 ...
 */
export function quoteByIdWhere(
  user: SessionUser,
  quoteId: string,
  actingOrgId?: string | null
): { ok: true; where: { id: string; organizationId?: string } } | { ok: false; reason: "no_org" } {
  const orgId = actingOrgId !== undefined ? actingOrgId : getEffectiveOrganizationId(user);
